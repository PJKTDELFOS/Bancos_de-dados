...
def ver_dados():
    while True:
        todos=input('digite o que deseja,[A] para visualização geral ou [B] pára ver registro unico:').strip().upper()
        if todos=='A':
            conn = conectar()
            cursor = conn.cursor()
            ...
            conn = conectar()
            cursor = conn.cursor()
            cursor.execute("select * from usuarios where id =?", (indice,))# sempre se
